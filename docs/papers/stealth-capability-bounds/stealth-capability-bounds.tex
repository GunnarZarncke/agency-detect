...
The usual UAD problem~\cite{uad2025} asks: given a multivariate dynamical trace \(X_t=(X_1(t),\ldots,X_n(t))\), find a cluster \(C\) admitting internal, sensory, active, and external parts
\[
...
Let \(G_\sigma(C)\) be an agency-gain term, for example predictive/control information or intentional compression~\cite{eis2025}. Define
\begin{equation}
\bar J_\Sigma(C)
=
...
We use the competence functional~\cite{biq2025}
\begin{equation}
B
=
I_{\mathrm{pred}}
...
p=P^\pi(H).
\]
By data processing~\cite{cover2006elements},
\begin{equation}
d_{\mathrm{Bern}}(p\|q)
=
p\log\frac pq
...
Camouflage is dangerous because it can preserve local usefulness while reducing local detectability~\cite{abc2025}. The hard bound is distributional: substantial observer-visible value shift requires KL divergence~\cite{cover2006elements}, unless the value shift occurs in unmonitored variables or accumulates slowly. Slow plotting is dangerous because rate bounds do not imply stock bounds. A small positive hidden BIQ rate can eventually cross a threshold when decay, reset, resource caps, and cumulative detection are absent.
...
but:
\begin{align*}
&\text{``What is the maximum hidden BIQ and accumulated resource} \\
&\text{consistent with all monitored filters, tags, decay rules, resets, and ledgers?''}
\end{align*}
...
R_{\mathrm{untag}}.
\]
The correct adversarial UAD target is therefore not merely a recovered boundary, but a certified upper bound on hidden productive BIQ and accumulated autonomous resource~\cite{friston2010free,kaelbling1998pomdp,biehl2022pomdp,Tishby2000IB}. In this framing, countermeasures work by reducing \(C_{\mathrm{hid},\Sigma}\), \(U_\Sigma\), \(b\), \(\tau\), \(\rho_U\), \(\rho_L\), and \(R_{\mathrm{untag}}\), while increasing \(q_\Sigma\), \(\lambda\), and cumulative evidence retention.
